...
const fonts = [
  {
    name: "IBM Plex Serif",
    data: await loadFont(
      "fonts/ibm-plex-serif/ibm-plex-serif-v20-latin-700.woff2",
    ),
    weight: 700,
    style: "normal",
  },
  {
    name: "JetBrains Mono",
    data: await loadFont("fonts/jetbrains-mono/JetBrainsMono-Regular.woff2"),
    weight: 400,
    style: "normal",
  },
  {
    name: "JetBrains Mono",
    data: await loadFont("fonts/jetbrains-mono/JetBrainsMono-Bold.woff2"),
    weight: 700,
    style: "normal",
  },
];

const files = (await markdownFiles(postsDir)).filter(
  (file) => path.basename(file) !== "_index.md",
);
...
for (const file of files) {
  ...
  const svg = await satori(card({ title, date, tags }), {
    width: 1200,
    height: 630,
    fonts,
  });
